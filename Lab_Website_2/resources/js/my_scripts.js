...
function viewStudentStats(id,yn){
	if(yn == 1){
		document.getElementById(id).style.visibility = "visible";
		//document.getElementById(id).style.height = 0;
  }else{
    document.getElementById(id).style.visibility = "hidden";
		//document.getElementById(id).style.visibility = auto;
  }
}
...
function loadStatsPage(){
	...
	var winnum = 0;
	var lossnum = 0;
	for(row_counter = 2; row_counter < stats_table.rows.length; row_counter++){
		if(parseInt(stats_table.rows[row_counter].cells[2].innerHTML) > parseInt(stats_table.rows[row_counter].cells[3].innerHTML)){
			stats_table.rows[row_counter].cells[4].innerHTML = "CU Boulder";
			winnum++;
		}else{
			stats_table.rows[row_counter].cells[4].innerHTML = stats_table.rows[row_counter].cells[1].innerHTML;
			lossnum++;
		}
	}
...
function loadPlayersPage(){
	for(rcounter = 0; rcounter < players.length; rcounter++){
		var x = document.getElementById("player_selector");
		cell1 = document.createElement("myAnchor");
		cell1.innerHTML = '<a id=“myAnchor” href="#" onclick="switchPlayers(0)">' + players[rcounter].name + "\r" +'</a>';//if i use rcounter when clicked it uses value 4 and therefore doesn't work... i can't figure out static variables either, the code is otherwise functional
		x.appendChild(cell1);
		// var y = document.createElement("players[rcounter].name");
		// y.setAttribute('onclick', 'switchPlayers(playerNum)');
		// y.setAttribute('href', '#');
		// x.appendChild(y);
		// var option = document.createElement("option");
		// option.text = players[rcounter].name;
		// x.add(option);
		// var option = document.createElement('option');
    // option.text = option.value = i;
    // x.add(option, 0);
		// <a href=#>players[rcounter].name</a>
